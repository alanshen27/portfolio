...
function MetricCard({ metric, index }: { metric: Metric; index: number }) {
  ...
  return (
    <motion.li
      ...
      className="group border-line bg-bg-elevated/80 relative overflow-hidden border p-5 backdrop-blur-sm md:p-6"
    >
      <div
      ...
      <p className="text-ink relative mt-4 font-[family-name:var(--font-display)] text-[clamp(2rem,4vw,2.75rem)] leading-none tracking-tight tabular-nums">
        {metric.prefix}
        {count}
        {metric.suffix}
      </p>
      <p className="text-ink-soft relative mt-2 text-sm">{metric.detail}</p>
    </motion.li>
  )
}
