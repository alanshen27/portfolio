...
export function AppPlate({
  ...
  screens: Screen[]
  ratio?: string
  /** Full-width layout: frame on the left, callout key alongside */
  spread?: boolean
  className?: string
}) {
  ...
  const key = (
    <figcaption
      className={`grid content-start ${spread ? 'gap-2' : 'mt-2 gap-1'}`}
    >
      {spread && (
        <p className="text-ink-faint border-line border-b pb-2 text-[13px]">
      ...
      {s.callouts.map((c, k) => (
        <p
          key={k}
          className={`text-ink-soft grid grid-cols-[1.25rem_1fr] leading-snug ${
            spread ? 'text-[15px]' : 'text-[13px]'
          }`}
        >
          <span className="text-accent font-medium tabular-nums">{k + 1}</span>
          <span>{c.text}</span>
        </p>
      ))}
    </figcaption>
  ...
  if (spread) {
    return (
      <figure
        className={`grid min-w-0 gap-6 md:grid-cols-12 md:gap-10 ${className}`}
      >
        <div className="min-w-0 md:col-span-8">{frame}</div>
        <div className="md:col-span-4">{key}</div>
      </figure>
    )
  }
  return (
